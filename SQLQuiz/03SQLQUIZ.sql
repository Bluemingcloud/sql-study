...
SELECT FIRST_NAME AS 이름, 
       ...
       CASE MOD(TRUNC((SYSDATE - HIRE_DATE) / 365), 5) WHEN 0 THEN '진급대상'
            ELSE NULL
       END AS 진급대상
FROM EMPLOYEES
...
SELECT FIRST_NAME AS 이름,
       TO_CHAR(HIRE_DATE, 'YYYY"년"MM"월"DD"일"') AS 입사일,
       TO_CHAR(SALARY * (1 + NVL(COMMISSION_PCT, 0)) * 1300, 'L999,999,999') AS 급여,
       DECODE(MOD(TO_CHAR(SYSDATE, 'YY') - TO_CHAR(HIRE_DATE, 'YY'), 5), 0, '진급대상') AS 진급대상
FROM EMPLOYEES
WHERE DEPARTMENT_ID IS NOT NULL;
